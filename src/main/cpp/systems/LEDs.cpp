...
void LEDs::Update(Robot::Mode mode, std::string alliance) {
  ...
  if(!xButtonPressed && Controllers::GetInstance().GetOperatorController().GetXButtonPressed()) {
      xButtonPressed = true;
    } else if (xButtonPressed && Controllers::GetInstance().GetOperatorController().GetXButtonPressed()) {
  ...
  if (mode == Robot::Mode::kDisabled) { //Turns the LEDs off when disabled.
    ...
    if (mode == Robot::Mode::kTeleop) { //Sets the LEDs when in TeleOp.
      m_red.ApplyTo(m_ledBuffer);

      if(xButtonPressed && (Controllers::GetInstance().GetOperatorController().GetLeftTriggerAxis() > 0.5)) { //Changes LEDs when Intake is activated.
    ...
    } if (mode == Robot::Mode::kAuto) { //Sets the LEDs when in Auto.
