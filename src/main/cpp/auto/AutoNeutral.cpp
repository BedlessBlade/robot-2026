...
AutoNeutral::AutoNeutral(frc::DriverStation::Alliance alliance) {
  /*
  this is test pseudo code for autos,  this one also uses setpoints but this auto might not be needed for comps and can be lowest on priority list for
  autos to finish
    m_tasks.push_back(std::make_shared<FollowPath>(
      std::vector<frc::Pose2d>{
          Locations::GetInstance().GetStartPosition(alliance, position), // position might be optional depending on how we want to start matches
